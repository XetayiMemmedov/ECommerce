...
public class CustomerManager
{
    ...
    public void ShowOrders()
    {
        ...
        {
            orders = orderService.GetAll(include: query => query.Include(o => o.Items).ThenInclude(oi => oi.Product));
        }
        else
        {
            orders = orderService.GetAll(predicate: x => x.OrderStatus == statustype, include:query=>query.Include(o=>o.Items).ThenInclude(oi=>oi.Product));
        }
    ...
    public void GiveFeedBack()
    {
        var orders = orderService.GetAll(
            ...
            .ThenInclude(oip=>oip.UserFeedBacks),
            predicate: x => x.OrderStatus == StatusType.Delivered && x.User.Id ==_userId
            );
        bool isexist = false;
                ...
                foreach (var item in orderitems)
                {
                    if (item.ProductId == productid)
                    {
                        ...
                        if (!inputmark.IsNullOrEmpty() && double.TryParse(inputmark, NumberStyles.Any, CultureInfo.InvariantCulture, out double productfeedback) && productfeedback > 0 && productfeedback <= 5)
                        {
                            ...
                            var userfeedback = _mapper.Map<UserFeedBack>(userfeedbackcreate);
